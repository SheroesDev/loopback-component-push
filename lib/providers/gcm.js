...
GcmProvider.prototype._setupPushConnection = function(options) {
  ...
  this._old_connection = new gcm.Sender(options.oldServerApiKey);
};

GcmProvider.prototype.pushNotification = function(notification, deviceToken, isOldGCMKey) {
  var self = this;
  ...
  //ADDED by Satyadeep on Jan 28, 2019
  //Because our old app is using a different GCM sender ID, compared to the new app after FCM migration
  // https://trello.com/c/lX7zLwo2/134-fcm-bbgo-gcm-key-issue  
  var connection = isOldGCMKey ? this._old_connection : this._connection;

  connection.send(message, registrationIds, 3, function(err, result) {
    ...
    debug('GCM result: %j', result, ' for device token ', registrationIds);
  });
};
